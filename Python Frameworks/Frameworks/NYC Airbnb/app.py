...
# Load data into CSV 
url = 'https://raw.githubusercontent.com/greenboi105/Python-Frameworks-and-Projects/main/Data%20Analysis/Data/AB_NYC_2019.csv'
...
hosts = [str(host) for host in top_host_df["Host_ID"].unique()]
host_plot = px.bar(top_host_df, hosts, y='P_Count', color="P_Count", title="Host IDs with Most Rentals", template="plotly_dark", labels=dict(x="Host Ids", P_Count="Available Listings"), height=700).update(layout=dict(title=dict(x=0.5)))
